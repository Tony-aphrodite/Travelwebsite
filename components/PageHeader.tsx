...
export default function PageHeader({
  ...
  return (
    <section className="pt-40 pb-16 bg-ivory-100 relative overflow-hidden">
      ...
      <div className="container-site relative z-10 text-center max-w-3xl">
        ...
        />
        {subtitle && <p className="text-[17px] text-charcoal-700">{subtitle}</p>}
        {crumbs.length > 0 && (
          <div className="flex justify-center gap-2 text-xs uppercase tracking-[0.1em] text-charcoal-500 mt-6">
            {crumbs.map((c, i) => (
              <span key={i} className="flex items-center gap-2">
                {c.href ? (
                  <Link href={c.href} className="text-plum-700 hover:underline">{c.label}</Link>
                ) : (
                  <span>{c.label}</span>
                )}
