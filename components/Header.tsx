...
function UserArea() {
  ...
  return (
    <>
      <Link
        href="/carrito"
        className="relative p-2.5 rounded-full text-ivory-50/85 hover:bg-ivory-50/10 hover:text-gold-500 transition-colors"
        title="Carrito"
      >
      ...
      <div className="relative">
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium text-ivory-50/85 hover:bg-ivory-50/10 hover:text-gold-500 transition-colors"
        >
          {user?.image ? (
...
function MobileAuthSection({ onClose }: { onClose: () => void }) {
  ...
  if (isLoggedIn) {
    return (
      <>
        <Link href="/cuenta" className="px-8 py-3 text-sm font-medium text-ivory-50/85 hover:bg-ivory-50/10 hover:text-gold-500" onClick={onClose}>
          Mi cuenta
        </Link>
        <Link href="/carrito" className="px-8 py-3 text-sm font-medium text-ivory-50/85 hover:bg-ivory-50/10 hover:text-gold-500" onClick={onClose}>
          Carrito
        </Link>
  ...
  return (
    <>
      <Link href="/auth/login" className="px-8 py-3 text-sm font-medium text-ivory-50/85 hover:bg-ivory-50/10 hover:text-gold-500" onClick={onClose}>
        Iniciar sesion
      </Link>
      <Link href="/auth/registro" className="px-8 py-3 text-sm font-medium text-gold-500 font-semibold hover:bg-ivory-50/10" onClick={onClose}>
        Registrarse gratis
      </Link>
    </>
  );
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ease-out ${
        scrolled
          ? 'py-3 bg-plum-900/85 backdrop-blur-xl border-b border-ivory-50/10 shadow-soft-md'
          : 'py-5 bg-plum-900/40 backdrop-blur-xl border-b border-transparent'
      }`}
    >
      <div className="max-w-[1440px] mx-auto px-6 lg:px-10 flex items-center justify-between gap-6">
        <Link href="/" className="flex items-center gap-2">
          ...
          <span className="font-display text-[1.6rem] font-bold text-ivory-50 tracking-tight">
            Aurelia
          </span>
        </Link>
        ...
        <nav className="hidden lg:flex items-center gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="relative text-sm font-medium text-ivory-50/85 hover:text-gold-500 transition-colors py-2 group"
            >
              {link.label}
              <span className="absolute left-1/2 -translate-x-1/2 bottom-0 w-0 h-px bg-gold-500 group-hover:w-full transition-all duration-500" />
            </Link>
          ))}
        </nav>
      ...
      {mobileOpen && (
        <div className="lg:hidden absolute top-full left-0 right-0 bg-plum-900/95 backdrop-blur-xl border-b border-ivory-50/10 shadow-soft-md">
          <nav className="flex flex-col py-4">
            {NAV_LINKS.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="px-8 py-3 text-sm font-medium text-ivory-50/85 hover:bg-ivory-50/10 hover:text-gold-500"
                onClick={() => setMobileOpen(false)}
              >
                {link.label}
              </Link>
            ))}
            <hr className="my-2 mx-6 border-ivory-50/10" />
